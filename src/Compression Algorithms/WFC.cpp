#include "../../headers/Compression Algorithms/WFC.h"
...
void WFC::updateWeightedList(unsigned short newIndex) {
    if (newIndex >= 256) throw IndexError();
    ...
    while (newIndex > 0) {
        ...
        else break;    // If the weight is < or = no exchange is needed.    
        newIndex--;
    }
}
...
bool WFC::isWeightHigher(unsigned short pos1,
    ...
    unsigned short previousCharWeight = charFrequencies[weightedList[pos2]];

    return newCharWeight > previousCharWeight;
